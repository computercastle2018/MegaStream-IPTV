...
@Serializable
data class MegaStreamPluginManifest(
    ...
    val supportsConfigurationActivity: Boolean
        get() = hasCapability(MegaStreamPluginContract.CAPABILITY_CONFIGURATION_ACTIVITY) &&
            !configurationActivityAction.isNullOrBlank()

    val usesActivityConfiguration: Boolean
        get() = configurationMode == MegaStreamPluginContract.CONFIGURATION_MODE_ACTIVITY

    val supportsHostRenderedConfiguration: Boolean
        get() = configurationMode == MegaStreamPluginContract.CONFIGURATION_MODE_HOST_SCHEMA ||
            (configurationMode != MegaStreamPluginContract.CONFIGURATION_MODE_ACTIVITY &&
                hasCapability(MegaStreamPluginContract.CAPABILITY_CONFIGURATION_SCHEMA))

    val canConfigure: Boolean
        get() = supportsHostRenderedConfiguration || supportsConfigurationActivity
}

data class InstalledMegaStreamPlugin(
    val packageName: String,
    val serviceClassName: String,
    val appLabel: String,
    val manifest: MegaStreamPluginManifest,
    val enabled: Boolean,
